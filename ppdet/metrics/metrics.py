...
def _save_json_atomic(obj, path):
    """Best-effort JSON save. Disk errors (e.g. EXT4 errno 117) must not abort eval.

    Prefer writing under /tmp then copying to ``path`` when the target filesystem
    is unhealthy; evaluation itself uses in-memory results.
    """
    path = os.path.abspath(path)
    tmp_dir = tempfile.gettempdir()
    safe_tmp = os.path.join(tmp_dir, 'paddledet_' + os.path.basename(path) + '.tmp')
    ...
    def _dump(dst):
        ...
        with open(dst, 'w', encoding='utf-8') as f:
            ...
            try:
                ...
                # errno 117 Structure needs cleaning, etc.
                logger.warning('fsync skipped for {}: {}'.format(dst, e))

    try:
        _dump(safe_tmp)
        try:
            shutil.copyfile(safe_tmp, path)
            try:
                os.remove(safe_tmp)
            except OSError:
                pass
            ...
            logger.warning(
                'Cannot write {} ({}). Keep a copy at {} for inspection.'.format(
                    path, e, safe_tmp))
            return safe_tmp
    except OSError as e:
        logger.warning('Skip saving prediction json {}: {}'.format(path, e))
        return None
...
class COCOMetric(Metric):
    ...
    def accumulate(self):
        if len(self.results['bbox']) > 0:
            ...
            # Evaluate from in-memory list; disk save is best-effort only
            saved = _save_json_atomic(self.results['bbox'], output)
            if saved:
                logger.info('The bbox result is saved to {}.'.format(saved))
            else:
                logger.warning(
                    'bbox.json not saved (disk error); continue eval in-memory.')
        ...
        if len(self.results['mask']) > 0:
            ...
            if self.output_eval:
                output = os.path.join(self.output_eval, output)
            saved = _save_json_atomic(self.results['mask'], output)
            if saved:
                logger.info('The mask result is saved to {}.'.format(saved))
        ...
        if len(self.results['segm']) > 0:
            ...
            if self.output_eval:
                output = os.path.join(self.output_eval, output)
            saved = _save_json_atomic(self.results['segm'], output)
            if saved:
                logger.info('The segm result is saved to {}.'.format(saved))
        ...
        if len(self.results['keypoint']) > 0:
            ...
            if self.output_eval:
                output = os.path.join(self.output_eval, output)
            saved = _save_json_atomic(self.results['keypoint'], output)
            if saved:
                logger.info('The keypoint result is saved to {}.'.format(saved))
