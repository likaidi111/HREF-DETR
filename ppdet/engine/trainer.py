...
class Trainer(object):
    ...
    def train(self, validate=False):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            eval_epoch = self.cfg.get('eval_epoch', self.cfg.snapshot_epoch)
            is_eval = validate and (
                (epoch_id + 1) % eval_epoch == 0 or epoch_id == self.end_epoch - 1)
            use_ema_weight = (is_snapshot or is_eval) and self.use_ema
            if use_ema_weight:
    ...
    def evaluate(self):
        ...
        if self.cfg.get('fleet', False):
            ...
            self.model = paddle.DataParallel(model, find_unused_parameters=True)
        with paddle.no_grad():
