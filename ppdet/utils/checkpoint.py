...
def _infer_last_epoch_from_path(path):
    """Checkpoint 名为 epoch_id 时，保存时 last_epoch = epoch_id + 1。"""
    base = os.path.basename(_strip_postfix(path))
    if base.isdigit():
        return int(base) + 1
    return 0


def _atomic_paddle_save(obj, path):
    """先写到 /tmp 再复制到目标路径，降低 /mnt 盘损坏导致权重丢失的概率。"""
    import tempfile
    import shutil
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='paddledet_ckpt_', suffix='.tmp', dir=tempfile.gettempdir())
    os.close(fd)
    try:
        paddle.save(obj, tmp_path)
        try:
            shutil.copyfile(tmp_path, path)
        except OSError as e:
            # Fallback: try direct save on target (may still fail on bad fs)
            logger.warning(
                'Copy checkpoint to {} failed ({}). Try direct save.'.format(
                    path, e))
...
def _optimizer_state_shape_mismatch(optimizer, optim_state_dict):
    """AdamW 动量张量形状与当前可训练参数不一致时，set_state_dict 可能成功但 step 会失败。"""
    param_list = getattr(optimizer, '_parameter_list', None)
    if not param_list:
        return False

    param_shapes = {tuple(p.shape) for p in param_list}
    param_count = len(param_list)
    moment_tensors = 0
    bad_shapes = 0

    skip_keys = {'LR_Scheduler', 'last_epoch'}
    for key, value in optim_state_dict.items():
        if key in skip_keys:
            continue
        if hasattr(value, 'shape') and len(value.shape) > 0:
            moment_tensors += 1
            if tuple(value.shape) not in param_shapes:
                bad_shapes += 1
        elif isinstance(value, dict):
            for sub_val in value.values():
                if hasattr(sub_val, 'shape') and len(sub_val.shape) > 0:
                    moment_tensors += 1
                    if tuple(sub_val.shape) not in param_shapes:
                        bad_shapes += 1

    if bad_shapes > 0:
        return True

    # AdamW: 每个可训练参数通常对应 4 份状态（moment1/2 + beta1/2 pow acc）
    if moment_tensors > 0 and param_count > 0:
        ratio = moment_tensors / float(param_count)
        if abs(ratio - round(ratio)) > 1e-3 or int(round(ratio)) < 2:
            return True
        if moment_tensors % param_count != 0:
            return True
    return False


def _skip_optimizer_resume(optimizer, pdopt_path, path, ema, ema_state_dict):
    """Skip incompatible/corrupt pdopt; keep model weights and infer resume epoch."""
    logger.warning(
        'Skip optimizer state from {} (incompatible with current model). '
        'Model weights are loaded; optimizer/LR will re-init.'.format(pdopt_path))
    last_epoch = _infer_last_epoch_from_path(path)
    if last_epoch > 0:
        logger.warning(
            'Inferred resume epoch {} from checkpoint name.'.format(last_epoch))
    if ema_state_dict is not None:
        ema.resume(ema_state_dict)
    return last_epoch


def _safe_paddle_load(path, desc='weights'):
    """Load paddle file; return None if missing/corrupt."""
    if not os.path.exists(path):
        return None
    try:
        return paddle.load(path)
    except Exception as e:
        logger.warning('Failed to load {} from {} ({})'.format(desc, path, e))
        return None
...
def load_weight(model, weight, optimizer=None, ema=None, exchange=True):
    ...
    pdema_path = path + '.pdema'
    if not os.path.exists(pdparam_path) and not os.path.exists(pdema_path):
    ...
    if ema is not None and os.path.exists(pdema_path):
        pdparams_dict = _safe_paddle_load(pdparam_path, 'pdparams')
        pdema_dict = _safe_paddle_load(pdema_path, 'pdema')
        if pdparams_dict is None and pdema_dict is None:
            raise ValueError(
                '`paddle.load` can not parse pdparams/pdema under {}'.format(
                    path))
        # Prefer valid files; if one is corrupt, reuse the other for both.
        if exchange:
            logger.info('Exchange model and ema_model to load:')
            ema_state_dict = pdparams_dict if pdparams_dict is not None else pdema_dict
            param_state_dict = pdema_dict if pdema_dict is not None else pdparams_dict
            logger.info('Loading ema_model weights from {}'.format(
                pdparam_path if pdparams_dict is not None else pdema_path))
            logger.info('Loading model weights from {}'.format(
                pdema_path if pdema_dict is not None else pdparam_path))
            if pdparams_dict is None or pdema_dict is None:
                logger.warning(
                    'One of pdparams/pdema is corrupt; using the valid file for both model and ema.'
                )
        else:
            ema_state_dict = pdema_dict if pdema_dict is not None else pdparams_dict
            param_state_dict = pdparams_dict if pdparams_dict is not None else pdema_dict
            logger.info('Loading ema_model weights from {}'.format(
                pdema_path if pdema_dict is not None else pdparam_path))
            logger.info('Loading model weights from {}'.format(
                pdparam_path if pdparams_dict is not None else pdema_path))
        ...
        param_state_dict = _safe_paddle_load(pdparam_path, 'pdparams')
        if param_state_dict is None:
            param_state_dict = _safe_paddle_load(pdema_path, 'pdema')
        if param_state_dict is None:
            raise ValueError(
                '`paddle.load` can not parse the file:{}'.format(pdparam_path))
        ema_state_dict = None
    ...
    last_epoch = 0
    pdopt_path = path + '.pdopt'
    if optimizer is not None and os.path.exists(pdopt_path):
        try:
            optim_state_dict = paddle.load(pdopt_path)
            ...
            if _optimizer_state_shape_mismatch(optimizer, optim_state_dict):
                last_epoch = _skip_optimizer_resume(
                    optimizer, pdopt_path, path, ema, ema_state_dict)
            else:
            ...
            logger.warning(
                'Failed to load optimizer state from {} ({}). '
                'Model weights are loaded; optimizer/LR will re-init.'.format(
                    pdopt_path, e))
            last_epoch = _infer_last_epoch_from_path(path)
            if last_epoch > 0:
                logger.warning(
                    'Inferred resume epoch {} from checkpoint name.'.format(
                        last_epoch))
            if ema_state_dict is not None:
                ema.resume(ema_state_dict)
        ...
        if last_epoch > 0:
            logger.warning(
                'Optimizer state {} not found; inferred resume epoch {}.'.format(
                    pdopt_path, last_epoch))
        if ema_state_dict is not None:
            ema.resume(ema_state_dict)
...
def save_model(model,
    ...
    if isinstance(model, nn.Layer):
        _atomic_paddle_save(model.state_dict(), save_path + ".pdparams")
        best_model = model.state_dict()
    else:
        assert isinstance(model,
                          dict), 'model is not a instance of nn.layer or dict'
        if ema_model is None:
            _atomic_paddle_save(model, save_path + ".pdparams")
            best_model = model
        else:
            ...
            _atomic_paddle_save(ema_model, save_path + ".pdparams")
            _atomic_paddle_save(model, save_path + ".pdema")
            best_model = ema_model

    if save_name == 'best_model':
        best_model_path = os.path.join(best_model_path, 'model')
        _atomic_paddle_save(best_model, best_model_path + ".pdparams")
    # save optimizer
    state_dict = optimizer.state_dict()
    state_dict['last_epoch'] = last_epoch
    _atomic_paddle_save(state_dict, save_path + ".pdopt")
