...
class HybridEncoder(nn.Layer):
    ...
    def __init__(self,
        ...
        print('[Ablation] HybridEncoder use_dsf={}'.format(self.use_dsf))
        self.dsf_modules = None
        if use_dsf:
            num_pan = max(0, len(in_channels) - 1)
            self.dsf_modules = nn.LayerList([
                DSF(in_channels=hidden_dim) for _ in range(num_pan)
            ])
    ...
    def forward(self, feats, for_mot=False, is_teacher=False):
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            if self.use_dsf and self.dsf_modules is not None:
                fused = self.dsf_modules[idx](feat_height, downsample_feat)
                pan_in = paddle.concat([downsample_feat, fused], axis=1)
            else:
                pan_in = paddle.concat(
                    [downsample_feat, feat_height], axis=1)
            out = self.pan_blocks[idx](pan_in)
            outs.append(out)
