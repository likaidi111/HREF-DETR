...
class DSF(nn.Layer):
    ...
    def forward(self, fpn_feat, pan_feat):
        ...
        # ========== 步骤1: 自适应融合 ==========
        # 拼接两路特征
        concat_feat = paddle.concat([fpn_feat, pan_feat], axis=1)  # [B, 2C, H, W]
        
        # 生成自适应权重
        weights = self.fusion_weight(concat_feat)  # [B, 2, 1, 1]
        w1 = weights[:, 0:1, :, :]  # FPN 权重
        w2 = weights[:, 1:2, :, :]  # PAN 权重
        
        # 加权融合
        x = fpn_feat * w1 + pan_feat * w2  # [B, C, H, W]
