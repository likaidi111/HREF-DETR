...
from img_ops.preprocess import preprocess, Resize, NormalizeImage, Permute, PadStride, LetterBoxResize, WarpAffine, Pad, decode_image, CULaneResize
    ...
    def preprocess(self, image_list):
        ...
        for im_path in image_list:
            im, im_info = preprocess(
                im_path,
                preprocess_ops,
                decode_input_channel=self.pred_config.decode_input_channel,
                decode_alpha_composite=self.pred_config.decode_alpha_composite)
...
class PredictConfig():
    ...
    def __init__(self, model_dir, use_fd_format=False):
        ...
        self.decode_input_channel = int(
            yml_conf.get('decode_input_channel', 3))
        self.decode_alpha_composite = bool(
            yml_conf.get('decode_alpha_composite', True))
