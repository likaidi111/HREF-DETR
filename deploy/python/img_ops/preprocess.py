...
class CULaneResize(object):
    ...
    def __call__(self, im, im_info):
        ...
        im = cv2.resize(im, (self.img_w, self.img_h))
        im = im.astype(np.float32) / 255.
        im = im.transpose(2, 0, 1)
        return im, im_info


def preprocess(im_path, preprocess_ops, decode_input_channel=3, decode_alpha_composite=True):
    im_info = {
        'scale_factor': np.array([1., 1.], dtype=np.float32),
        'im_shape': None,
    }
    im, im_info = decode_image(im_path)
